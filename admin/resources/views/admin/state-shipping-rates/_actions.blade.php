<div class="d-flex gap-1">
    <button class="btn btn-sm btn-outline-primary btn-edit-rate"
            data-id="{{ $rate->id }}"
            data-state_name="{{ $rate->state_name }}"
            data-state_code="{{ $rate->state_code }}"
            data-base_rate="{{ $rate->base_rate }}"
            data-per_kg_rate="{{ $rate->per_kg_rate }}"
            data-free_shipping_above="{{ $rate->free_shipping_above }}"
            data-estimated_days_min="{{ $rate->estimated_days_min }}"
            data-estimated_days_max="{{ $rate->estimated_days_max }}"
    ...
    <button class="btn btn-sm btn-outline-danger btn-delete-rate" data-id="{{ $rate->id }}" data-name="{{ $rate->state_name }}">
